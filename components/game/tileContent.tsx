...
const TileContent: FC<TileContentProps> = ({ tileId, tilefamily_id, tileName }) => {
      ...
      <Center textAlign="center" height="full" width="full">
         {tilefamily_id || tilefamily_id === 0 ? (
            <Image
               ...
               alt="pink home"
               tintColor={
                  TileFamily.filter((family) => family.tilefamilyId === tilefamily_id)[0].color
               }
            />
         ...
         >
            {playersOnCase().map((player, index) => {
               return <Box key={index} h="3" w="3" rounded="full" backgroundColor={player.color} />
            })}
         </Row>
      </Center>
   )
